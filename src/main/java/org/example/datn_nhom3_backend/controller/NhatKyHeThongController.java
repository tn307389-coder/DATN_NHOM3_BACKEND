package org.example.datn_nhom3_backend.controller;
import jakarta.persistence.Id;
import org.example.datn_nhom3_backend.entity.NhatKyHeThong;
import org.example.datn_nhom3_backend.exception.ResourceNotFoundException;
import org.example.datn_nhom3_backend.service.NhatKyHeThongService;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.*;
import java.lang.reflect.Field;
import java.util.List;
import java.util.Optional;
@RestController
@RequestMapping("/api/nhat-ky-he-thong")
@CrossOrigin(origins = "http://localhost:5173")
public class NhatKyHeThongController {
    private final NhatKyHeThongService service;
    public NhatKyHeThongController(NhatKyHeThongService service) {
        this.service = service;
    }
    @GetMapping
    public List<NhatKyHeThong> getAll() {
        return service.getAll();
    }
    @GetMapping("/{id}")
    public ResponseEntity<NhatKyHeThong> getById(@PathVariable Integer id) {
        return service.getById(id)
                .map(ResponseEntity::ok)
                .orElseThrow(() -> new ResourceNotFoundException("Không tìm thấy dữ liệu với ID: " + id));
    }
    @PostMapping
    public NhatKyHeThong create(@RequestBody NhatKyHeThong data) {
        return service.save(data);
    }
    @PutMapping("/{id}")
    public NhatKyHeThong update(@PathVariable Integer id, @RequestBody NhatKyHeThong data) throws IllegalAccessException {
        setEntityId(data, id);
        return service.save(data);
    }
    @DeleteMapping("/{id}")
    public void delete(@PathVariable Integer id) {
        service.delete(id);
    }
    private void setEntityId(NhatKyHeThong data, Integer id) throws IllegalAccessException {
        for (Field field : data.getClass().getDeclaredFields()) {
            if (field.isAnnotationPresent(Id.class)) {
                field.setAccessible(true);
                field.set(data, id);
                return;
            }
        }
    }
}
